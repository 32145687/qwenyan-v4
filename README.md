...
**Current Phase = P12.3 = COMPLETE / DELIVERED**（P12.2 Durable Workflow 完成并 FINAL ACCEPTED；**P12.3 TD1 Variant 实体级 Merge/Override = Final Acceptance ACCEPTED，Commit `b76986d`**：六类 Story State 经 EntityOverride 读穿透（INHERIT/OVERRIDE/REMOVE），Variant own 表承接 ADD（含 ADD 撞 id 隔离守卫），REMOVE→OVERRIDE 原子恢复（replaceOverride），saveOverride 严格 INSERT 语义保持，Memory 语义与 Workflow/Continuation/Facade 均不受影响；`StoryStateVariantMergeTest` 全绿 + 全量回归全 PASS。下一阶段为 **P13 Long-form Continuity Layer（规划 / NOT_STARTED，见 master-plan §32）**。）
