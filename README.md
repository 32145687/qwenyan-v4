...
| **P13**   | P13 Long-form Continuity Layer（LCL）：解决「单章 AI 写作工具 → 100+ 章连续创作」；Planning → LCL-A Narrative State → LCL-B Chapter Context Pack → LCL-C Foreshadow Lifecycle → LCL-D Reveal / Rolling Horizon；**COMPLETE / SEALED，Commit `f291853`**                                                                                                                                                                                                                                                                                                                                                                                      | ✅ COMPLETE / SEALED |
...
| P13 LCL-D | Reveal / Rolling Horizon（`Reveal` Reader-only Story State fact（无 `revealedTo`）+ Schema v9（`8.sqm`：Reveal 表 + Event/Foreshadow/TimelineEntry scope 索引）+ `RevealUseCases`（Variant-only / Original 只读 / duplicate→reject）+ `RollingHorizonCandidate/Projector`（bounded 确定性、无 LLM/正文）+ `RollingHorizonUseCases`（Task Checkpoint 承载）；边界：ChapterContextPack / NarrativeState / ForeshadowLifecycle / Resolver / Event·Timeline 物理语义均 NO CHANGE）；Commit `f291853`）                                                                                                                                                                                                                                                                                                                                                                | ✅ DELIVERED / SEALED     |
