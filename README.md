...
**Current Phase = P12.3 = COMPLETE**（P12.2 Durable Workflow 完成并 FINAL ACCEPTED；P12.3 TD1 Variant 实体级 Merge/Override 完成：六类 Story State 经 EntityOverride 读穿透（INHERIT/OVERRIDE/REMOVE），Variant own 表承接 ADD，Memory 语义与 Workflow/Continuation/Facade 均不受影响；`StoryStateVariantMergeTest`（OVERRIDE/REMOVE/INHERIT/ADD/ISOLATION/IDENTITY/RECOVER/Memory-scope/关联）+ 全量回归全 PASS，`./gradlew test` / `assembleDebug` / `git diff --check` 全 PASS。下一阶段（若存在规划则引用，否则 P12+ FUTURE 保持 🔮）。）
